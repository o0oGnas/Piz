...
public class ReferenceController {
    ...
    private void addListenerToTableItems() {
        ...
        tbvTable.itemsProperty().addListener(
                l -> executeVoidRunner("Error when handling change to item list property", () -> {
    ...
    private void bindTableViewItemListener() {
        ...
        if (itemList != null) {
            itemList.addListener((ListChangeListener<ReferenceModel>) l ->
                    executeVoidRunner("Error when handling changes to item list", this::handleTableViewItemChange));
        }
    }
    ...
    private void initialiseTableSelectionModel() {
        ...
        selectedList.addListener((ListChangeListener<ReferenceModel>) l ->
                executeVoidRunner("Error when handling changes to selected list",
    ...
    private void initialiseReferenceListListener() {
        applicationModel.getReferenceListProperty().addListener(
                propertyListener -> executeVoidRunner("Error when handling reference list property change",
    ...
    private void addReferenceListListener() {
        applicationModel.getReferenceList().addListener((ListChangeListener<ReferenceModel>) l ->
                executeVoidRunner("Error when handling changes to reference list", () -> {
    ...
    private ObservableList<ReferenceModel> getFilteredList() {
        ...
        for (ReferenceModel reference : applicationModel.getReferenceList()) {
            ...
            boolean checkDate =
                    dtpFrom.getDateTimeValue().compareTo(date) <= 0 && date.compareTo(dtpTo.getDateTimeValue()) <= 0;
